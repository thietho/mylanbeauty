...
$tooltip.css({borderColor:'#CCC'}).find('.stickystatus:eq(0)').css({background:this.stickybordercolors[0]}).html(this.stickynotice1)}},docktooltip:function($,$tooltip,e){this.isdocked=true
$tooltip.css({borderColor:'#CCC'}).find('.stickystatus:eq(0)').css({background:this.stickybordercolors[1]}).html(this.stickynotice2)},init:function(targetselector,tipid){jQuery(document).ready(function($){var $targets=$(targetselector)
...
stickytooltip.init("*[data-tooltip]","mystickytooltip");$(document).ready(function(){$(".link_hover").hover(function(){var img='<img src="'+$(this).attr('ref')+'">';var text='<h5>'+$(this).attr('title')+'</h5>';$('#sticky1').html(img+text);});});
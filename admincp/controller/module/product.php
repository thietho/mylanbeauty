...
class ControllerModuleProduct extends Controller
{
    ...
    private function productData()
    {
        ...
        $display = urldecode($this->request->get['display']);
        if($display !="")
        {
            $where .= " AND status like '".$display."'";
        }
    ...
    public function getList($template="module/product_list.tpl")
    {
            ...
            //foreach($rows as $i => $media)
        {
            ...
            foreach($data_child as $key =>$child)
            {
                ...
                $data_child[$key]['link_edit'] = $this->url->http('module/product/update&sitemapid='.$sitemap['sitemapid'].'&mediaid='.$child['mediaid']);
